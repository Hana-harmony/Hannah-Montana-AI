...
분석 응답은 이벤트·감성·중요도·종목 매핑 confidence와 `review_required`, `review_reasons`를 포함해 낮은 신뢰도 결과를 자동 발송 전에 차단할 수 있게 한다.
...
분석 API는 응답 계약을 유지하면서 내부 audit log에 모델 버전, latency, 예측 라벨, confidence, review flag, 입력 hash를 남긴다.
