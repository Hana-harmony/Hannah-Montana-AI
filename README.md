...
현재 운영 모델은 Naver News Search 발견 데이터와 OpenDART 공시검색 row를 v1 baseline으로 유지하되, v2에서는 사용 허가된 뉴스 원문과 OpenDART document 전문 507건을 저장·학습한다. 사람이 검수하지 않은 전문 약한 라벨은 이벤트/종목 맥락 학습에만 쓰고 감성·중요도 supervised loss에서는 제외한다.
