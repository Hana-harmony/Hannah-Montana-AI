...
def test_model_release_report_matches_source_reports() -> None:
    ...
    assert release_report["training"]["sample_count"] == 4711
    assert release_report["training"]["pseudo_labeled_sample_count"] == 1102
    assert release_report["quality_gates"]["real_news_gold"]["sample_count"] == 80
    assert release_report["pseudo_labeling"]["weak_label_accepted_count"] == 340
    assert release_report["pseudo_labeling"]["stock_candidate_accepted_count"] == 762
    ...
    assert stock_candidate_labeling["candidate_count"] == 15041
    assert stock_candidate_labeling["accepted_count"] == 762
    assert stock_candidate_labeling["accepted_stock_count"] == 762
    assert stock_candidate_labeling["accepted_count_by_primary_label"]["RISK"] == 290
...
def test_stock_candidate_quota_experiment_report_tracks_release_expansion() -> None:
    ...
    assert profiles["current_release"]["overall_status"] == "pass"
    assert profiles["current_release"]["stock_candidate_labeling"]["accepted_count"] == 762
    assert profiles["current_release"]["stock_candidate_labeling"][
        "accepted_stock_count"
    ] == 762
    assert profiles["previous_release"]["stock_candidate_labeling"]["accepted_count"] == 500
    assert report["best_promotable_profile"]["accepted_stock_count"] == 762
...
def test_pseudo_label_monitoring_report_matches_source_reports() -> None:
    ...
    assert monitoring_report["candidate_funnel"]["raw_candidate_count"] == 65460
    assert monitoring_report["candidate_funnel"]["high_signal_candidate_count"] == 5163
    assert monitoring_report["candidate_funnel"]["promoted_count"] == 1102
    assert (
