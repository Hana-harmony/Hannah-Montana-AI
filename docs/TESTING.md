...
- `reports/weak-distillation-report.json`은 57,045건 약지도 후보 중 5,050건을 고신호 후보로 선별하고, teacher gate를 통과한 weak-label 358건과 종목 후보 702건을 이벤트 모델 학습에 승격한 결과를 기록한다.
...
- `reports/model-release-report.json`은 현재 모델 버전 `financial-ml-tfidf-logreg-20260610183402`의 전체 release gate와 pseudo-label consistency check를 `overall_status=pass`로 기록한다.
- `reports/pseudo-label-promotion-monitoring.json`은 고신호 후보 5,050건, teacher 탈락 3,747건, quota 보류 243건, 최종 승격 1,060건을 `overall_status=pass`로 기록한다.
...
- `reports/stock-collection-shard-plan.json`은 후보 큐와 gold가 없는 878개 종목, 9개 shard, 4,390개 Naver 쿼리를 기록한다.
- stock collection shard plan은 702개 `no_raw_no_candidate` 종목을 raw가 이미 있는 종목보다 먼저 수집 대상으로 둔다.
- `reports/stock-candidate-quota-experiment.json`은 calibrated current release 702건/702종목이 gate를 통과했고, risk/contract 확장 profile은 863건/664종목까지 확장됐지만 실제 뉴스 gold macro F1 gate를 통과하지 못했음을 기록한다.
- `reports/stock-training-candidate-report.json`은 검수 대기 후보 12,481건, 3,086개 종목을 기록하며 coverage gate를 `pass`로 기록한다.
