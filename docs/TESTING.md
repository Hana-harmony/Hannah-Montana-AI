...
## 현재 ML 검증 기준
- `reports/ml-training-report.json`은 3,609건 supervised 샘플, 918건 pseudo-label 샘플, 722건 supervised holdout 검증 결과를 기록한다.
- `reports/weak-distillation-report.json`은 43,269건 약지도 후보 중 4,925건을 고신호 후보로 선별하고, teacher gate를 통과한 weak-label 360건과 종목 후보 558건을 이벤트 모델 학습에 승격한 결과를 기록한다.
...
- 현재 실제 뉴스 gold 결과는 이벤트 recall 0.9625, 이벤트 macro F1 0.9217, 감성 accuracy 0.9125, 중요도 accuracy 0.9250, 종목 accuracy 1.0이다.
- `reports/model-release-report.json`은 현재 모델 버전 `financial-ml-tfidf-logreg-20260604212852`의 전체 release gate와 pseudo-label consistency check를 `overall_status=pass`로 기록한다.
- `reports/pseudo-label-promotion-monitoring.json`은 고신호 후보 4,925건, teacher 탈락 3,395건, quota 보류 612건, 최종 승격 918건을 `overall_status=pass`로 기록한다.
- `reports/stock-coverage-report.json`은 universe 3,967개, raw 매칭 2,666개 종목, supervised 38개 종목, evaluation 57개 종목을 기록한다.
- `reports/stock-coverage-report.json`은 event-model-only pseudo 학습 coverage 558건, 558개 종목도 별도 섹션으로 기록한다.
- `reports/stock-collection-shard-plan.json`은 후보 큐와 gold가 없는 1,507개 종목, 16개 shard, 7,535개 Naver 쿼리를 기록한다.
- stock collection shard plan은 1,297개 `no_raw_no_candidate` 종목을 raw가 이미 있는 종목보다 먼저 수집 대상으로 둔다.
- `reports/stock-candidate-quota-experiment.json`은 이전 release 500건/500종목, risk/contract 확장 693건/516종목, calibrated current release 558건/558종목을 함께 기록한다.
- `reports/stock-training-candidate-report.json`은 검수 대기 후보 8,291건, 2,456개 종목을 기록하며 coverage gate를 `pass`로 기록한다.
