...
## 현재 ML 검증 기준
- `reports/ml-training-report.json`은 3,609건 supervised 샘플, 580건 pseudo-label 샘플, 722건 supervised holdout 검증 결과를 기록한다.
- `reports/weak-distillation-report.json`은 37,278건 약지도 후보 중 4,845건을 고신호 후보로 선별하고, teacher gate를 통과한 weak-label 360건과 종목 후보 220건을 이벤트 모델 학습에 승격한 결과를 기록한다.
