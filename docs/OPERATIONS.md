...
```
- `reports/stock-collection-shard-plan.json`은 현재 누락 후보 1,160개 종목, 12개 shard, 5,800개 Naver 쿼리를 기록한다.
...
- 현재 artifact는 48,992건 수집 후보 중 weak-label 360건과 종목 후보 큐 633건을 이벤트 모델 학습에 반영했다.
- 종목 후보 큐 승격분은 per-stock quota 1건으로 제한해 633건이 633개 종목에 분산되도록 한다.
...
- 현재 48,992건 raw 후보 중 4,978건이 고신호 후보이고, teacher gate에서 3,587건이 탈락하며 weak-label 360건과 종목 후보 633건만 student 이벤트 모델 학습에 승격된다.
...
- 현재 event model pseudo training coverage는 633건, 633개 종목이며 supervised gold coverage로 간주하지 않는다.
...
- 현재 shard plan은 977개 `no_raw_no_candidate` 종목과 183개 `raw_without_candidate` 종목을 우선 수집 대상으로 둔다.
...
- 후보 큐 2,804개 종목에서 종목·라벨별 human review batch 운영
- 재학습 기준과 rollback 절차
