...
```
- `reports/stock-collection-shard-plan.json`은 현재 누락 후보 1,397개 종목, 14개 shard, 6,985개 Naver 쿼리를 기록한다.
...
- 현재 artifact는 44,997건 수집 후보 중 weak-label 360건과 종목 후보 큐 580건을 이벤트 모델 학습에 반영했다.
- 종목 후보 큐 승격분은 per-stock quota 1건으로 제한해 580건이 580개 종목에 분산되도록 한다.
...
- 현재 44,997건 raw 후보 중 4,952건이 고신호 후보이고, teacher gate에서 3,479건이 탈락하며 weak-label 360건과 종목 후보 580건만 student 이벤트 모델 학습에 승격된다.
...
- 현재 event model pseudo training coverage는 580건, 580개 종목이며 supervised gold coverage로 간주하지 않는다.
...
- 현재 shard plan은 1,206개 `no_raw_no_candidate` 종목과 191개 `raw_without_candidate` 종목을 우선 수집 대상으로 둔다.
- `reports/stock-candidate-quota-experiment.json`은 이전 release 500건/500종목과 calibrated current release 580건/580종목은 gate 통과, risk/contract 확장 716건/536종목은 실제 뉴스 gold macro F1 미달로 gate 실패했음을 기록한다.
...
- 후보 큐 2,567개 종목에서 종목·라벨별 human review batch 운영
- 재학습 기준과 rollback 절차
