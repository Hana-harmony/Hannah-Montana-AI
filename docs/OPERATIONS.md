...
```
- `reports/stock-collection-shard-plan.json`은 현재 누락 후보 540개 종목, 6개 shard, 2,700개 Naver 쿼리를 기록한다.
...
- 현재 artifact는 65,460건 수집 후보 중 weak-label 340건과 종목 후보 큐 762건을 이벤트 모델 학습에 반영했다.
- 종목 후보 큐 승격분은 per-stock quota 1건으로 제한해 762건이 762개 종목에 분산되도록 한다.
...
- 현재 65,460건 raw 후보 중 5,163건이 고신호 후보이고, teacher gate에서 3,975건이 탈락하며 weak-label 340건과 종목 후보 762건만 student 이벤트 모델 학습에 승격된다.
...
- 현재 event model pseudo training coverage는 762건, 762개 종목이며 supervised gold coverage로 간주하지 않는다.
...
- 현재 shard plan은 424개 `no_raw_no_candidate` 종목과 116개 `raw_without_candidate` 종목을 우선 수집 대상으로 둔다.
...
- 후보 큐 3,424개 종목에서 종목·라벨별 human review batch 운영
- 재학습 기준과 rollback 절차
