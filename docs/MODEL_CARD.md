...
- 모델 구조: 한국·미국 종목명, 시장, 거래소, business keyword, 섹터, 산업, 사업모델, 규모 버킷, 동일업종 비교 종목, OpenDART 회사 업종코드, WiseReport 사업개요, 매출, 영업이익, 순이익, anchor profile을 cross-market profile corpus로 만들고 TF-IDF retrieval, TruncatedSVD semantic embedding, business profile classifier, 재무/업종/규모 feature, supervised pairwise LogisticRegression reranker를 저장한다.
...
- 최신 국내 업종 coverage: `data/reference/korea_stock_industries.csv`는 Naver 동일업종 비교 데이터 2,649개를 포함하며, OpenDART/WiseReport business profile ML 보강 후 전종목 학습 profile 기준 3,089개가 specific profile이다.
- business profile classifier: 2,639개 강한 라벨과 20개 WiseReport 약지도 라벨로 학습한 `FeatureUnion(TfidfVectorizer char/word)+LogisticRegression`이며, holdout accuracy 0.973485, macro F1 0.967205, weighted F1 0.974094를 기록한다. 발표용 요약은 `docs/GLOBAL_PEER_BUSINESS_PROFILE_CLASSIFIER.md`에 정리한다.
...
- 사용자 노출 문구 정책: `headline`/`summary`에는 내부 `confidence`, `similarity score`, ranker 점수를 쓰지 않는다. confidence와 score는 API/리포트의 품질 관측 메타데이터로만 유지한다. 글로벌 투자자용 설명은 anchor display name, 검수된 영어 표시명, 원천 영문명 순으로 한국 종목명을 영어로 노출한다. 검증된 영문명이 없으면 임의 번역을 만들지 않고 `Korean stock <code>`로 표시한다.
...
- 전종목 coverage 테스트: `reports/global-peer-full-coverage-report.json`은 한국 master 3,967개 전부를 실제 matcher에 넣어 추론 성공률, 동일회사 중복, matched factor 누락, LOW confidence, generic sector 비율을 검증한다. 최신 quality gate는 3,967/3,967개 성공으로 `pass`이고 LOW confidence 22.1326%도 confidence monitoring `pass`다. specific profile 3,089개 기준 LOW confidence는 0개다.
...
- 설명 생성: 피어 선택은 hybrid ranker가 담당하고, headline/summary는 구조화된 peer fact를 근거로 생성하는 Structured RAG 설명기로 분리했다. 기본 serving은 deterministic grounded template이며, `HANNAH_GLOBAL_PEER_EXPLANATION_MODE=local_llm`과 OpenAI-compatible local endpoint가 켜진 경우에만 Qwen3 설명 LLM을 사용한다. LLM 출력은 display name, peer명, ticker, 섹터/산업/사업모델, 금지 투자문구, headline exact match, summary exact match, 반복 단어 검증을 통과해야 하며 실패하면 template으로 fallback한다.
- Qwen3 설명 LLM: `data/training/global_peer_explanation_sft.jsonl` 3,967개 전종목 SFT 샘플로 `mlx-community/Qwen3-0.6B-4bit` LoRA를 학습했다. prompt version은 `global-peer-structured-rag-explainer-v7`, split은 train 3,571 / valid 198 / test 198이고, 500 iters, batch size 1, 8 layers, learning rate 1e-5 기준 final validation loss 0.000, test loss 0.000, test perplexity 1.000, peak memory 2.238GB를 기록했다.
- raw LLM 생성 검증: `scripts/evaluate_global_peer_qwen3_generation.py`로 대표 30종목을 실제 `mlx_lm.generate` 호출로 평가했다. v7 strict gate 기준 JSON valid 30/30, exact headline 30/30, exact summary 30/30, grounded 30/30, pass rate 100%를 기록했다. 이전 raw 직접 생성은 대표 10개 모두 grounded fail이었고, v4/v5/v6 개선 과정에서 이름 반복·미제공 숫자 생성·사용자 노출 점수 문구·한글 종목명 문제를 발견해 summary exact gate, 영어 표시명, 점수 비노출 guard를 추가했다.
- Codex 검수 보강: 회사명 표시용 `display_name`을 anchor에 추가하고, 미국 회사명 `Class A`, `When-Issued`, 쉼표 표기를 정규화한다. LLM이 canonical headline/summary와 다르게 생성하거나 제공되지 않은 숫자/회사명을 만들면 실제 API는 검수된 template으로 fallback한다.
- 산출물: `src/hannah_montana_ai/model_store/global_peer_ml.joblib`, `src/hannah_montana_ai/model_store/global_peer_qwen3_explainer_lora/adapters.safetensors`, `src/hannah_montana_ai/model_store/global_peer_qwen3_explainer_lora/adapter_config.json`, `reports/global-peer-training-report.json`, `reports/global-peer-ai-smoke-report.json`, `reports/global-peer-korea-industry-sync-report.json`, `reports/global-peer-korea-company-profile-sync-report.json`, `reports/global-peer-fundamentals-sync-report.json`, `reports/global-peer-full-coverage-report.json`, `reports/global-peer-all-results.json`, `reports/global-peer-all-results.csv`, `reports/global-peer-explanation-llm-readiness.json`, `reports/global-peer-qwen3-explainer-training.json`, `reports/global-peer-qwen3-generation-eval.json`, `docs/GLOBAL_PEER_ALL_RESULTS.md`, `docs/GLOBAL_PEER_BUSINESS_PROFILE_CLASSIFIER.md`, `data/reference/us_stock_universe.csv`, `data/reference/global_peer_fundamentals.csv`, `data/reference/korea_stock_industries.csv`, `data/reference/korea_company_profiles.csv`
- 재학습: `uv run python scripts/sync_us_stock_universe.py`로 미국 universe를 갱신하고, `uv run python scripts/sync_global_peer_fundamentals.py`, `uv run python scripts/sync_korea_stock_industries.py`, `uv run python scripts/sync_korea_company_profiles.py`로 한국·미국 재무/규모/업종/회사 사업개요 dataset을 갱신한 뒤, `uv run python scripts/train_global_peer_model.py`, `uv run python scripts/build_global_peer_ai_smoke_report.py`, `uv run python scripts/build_global_peer_full_coverage_report.py`, `uv run python scripts/build_global_peer_all_results_report.py`, `uv run python scripts/build_global_peer_explanation_llm_dataset.py`, `uv run --extra llm-training python scripts/train_global_peer_qwen3_explainer.py`, `uv run --extra llm-training python scripts/evaluate_global_peer_qwen3_generation.py --min-pass-rate 0.9`로 artifact와 report를 재생성한다.
