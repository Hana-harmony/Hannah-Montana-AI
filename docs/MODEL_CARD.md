...
## 모델명
`financial-ml-tfidf-logreg-20260610223509`
...
- 이번 artifact 학습 샘플 수: 4,728
- supervised 학습 샘플 수: 3,609
- teacher-gated pseudo-label 학습 샘플 수: 1,119
- 실제 수집 raw 총량: 67,286건
- 실제 수집 원천: OpenDART 공시검색 25,966건
- 실제 수집 원천: Naver News Search 41,320건
- 합성 증강 샘플 수: 1,656건
...
- 실제 뉴스 gold 샘플 수: 80건
- 약지도 후보 수: 67,286건
- distillation 통과 후보 수: 5,190건
- teacher confidence gate 통과 후 artifact 학습 승격 후보 수: 1,119건
- weak-label distillation 승격 수: 344건
- 종목 후보 큐 승격 수: 775건
- 종목 후보 큐 승격 종목 수: 775개
...
- raw 후보에서 보수적 종목명·종목코드 매칭으로 확인한 종목 수: 3,597개
- 학습 승격 후보 큐 샘플 수: 15,511건
- 학습 승격 후보 큐 종목 수: 3,485개
...
- 종목 후보 큐에서는 teacher gate와 종목별 quota를 통과한 `RISK` 298건, `CONTRACT` 260건, `CAPITAL_ACTION` 120건, `CORPORATE_ACTION` 44건, `EARNINGS` 30건, `MACRO` 23건을 이벤트 모델 학습에 추가했다.
- 종목 후보의 per-stock quota는 1건으로 제한해 775건이 775개 종목에 분산되도록 했다.
- `stock-candidate-quota-experiment`에서 current release는 775건/775종목으로 실제 뉴스 gold gate를 통과해 best profile로 선택했다. risk/contract per-stock 2 profile은 887건/705종목까지 늘었지만 실제 뉴스 gold macro F1 gate를 통과하지 못했다.
...
- 67,286건 raw 후보 중 5,190건이 고신호 후보로 남았다.
- teacher confidence 또는 weak-label 합의 기준에서 3,999건이 탈락했다.
- weak-label distillation에서는 `RISK` 140건, `CONTRACT` 180건, `CORPORATE_ACTION` 24건을 student 이벤트 모델 학습에 승격했다.
- 종목 후보 큐에서는 `RISK` 298건, `CONTRACT` 260건, `CAPITAL_ACTION` 120건, `CORPORATE_ACTION` 44건, `EARNINGS` 30건, `MACRO` 23건을 teacher gate로 추가 승격했다.
...
- raw 후보는 3,597개 종목까지 매칭되므로 다음 단계는 raw 후보를 종목별·라벨별로 검수해 supervised/gold 데이터로 승격하는 것이다.
- 후보 큐는 3,485개 종목을 포함하지만 약지도 기반 검수 대기 데이터이므로 gold label로 직접 사용하지 않는다.
- coverage review plan은 2,000개 종목의 검수 과제를 만들지만 아직 검수자 승인이 없으므로 supervised coverage gate를 통과시키지 않는다.
- 현재 artifact는 후보 큐 중 775개 종목의 775건만 teacher gate를 통과한 event-model-only pseudo-label로 제한 투입했다.
