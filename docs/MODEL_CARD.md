...
- Qwen3 설명 LLM: `data/training/global_peer_explanation_sft.jsonl` 3,967개 전종목 SFT 샘플로 `mlx-community/Qwen3-0.6B-4bit` LoRA를 학습했다. prompt version은 `global-peer-structured-rag-explainer-v6`, split은 train 3,571 / valid 198 / test 198이고, 500 iters, batch size 1, 8 layers, learning rate 1e-5 기준 final validation loss 0.000, test loss 0.000, test perplexity 1.000, peak memory 2.238GB를 기록했다.
- raw LLM 생성 검증: `scripts/evaluate_global_peer_qwen3_generation.py`로 대표 30종목을 실제 `mlx_lm.generate` 호출로 평가했다. v6 strict gate 기준 JSON valid 30/30, exact headline 30/30, exact summary 30/30, grounded 30/30, pass rate 100%를 기록했다. 이전 raw 직접 생성은 대표 10개 모두 grounded fail이었고, v4/v5 개선 과정에서 이름 반복·미제공 숫자 생성 문제를 발견해 summary exact gate와 fallback guard를 추가했다.
