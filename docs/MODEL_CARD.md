...
## 모델명
`financial-ml-tfidf-logreg-20260610183402`
...
- 실제 수집 raw 총량: 57,045건
- 실제 수집 원천: OpenDART 공시검색 25,966건
- 실제 수집 원천: Naver News Search 31,079건
- 합성 증강 샘플 수: 1,656건
...
- 학습 승격 후보 큐 샘플 수: 12,481건
- 학습 승격 후보 큐 종목 수: 3,086개
...
- 종목 후보 큐에서는 teacher gate와 종목별 quota를 통과한 `RISK` 258건, `CONTRACT` 237건, `CAPITAL_ACTION` 120건, `CORPORATE_ACTION` 44건, `EARNINGS` 28건, `MACRO` 15건을 이벤트 모델 학습에 추가했다.
- 종목 후보의 per-stock quota는 1건으로 제한해 702건이 702개 종목에 분산되도록 했다.
- `stock-candidate-quota-experiment`에서 current release는 702건/702종목으로 실제 뉴스 gold gate를 통과해 best profile로 선택했다. risk/contract per-stock 2 profile은 863건/664종목까지 늘었지만 실제 뉴스 gold macro F1 gate를 통과하지 못했다.
...
- 57,045건 raw 후보 중 5,050건이 고신호 후보로 남았다.
...
- 종목 후보 큐에서는 `RISK` 258건, `CONTRACT` 237건, `CAPITAL_ACTION` 120건, `CORPORATE_ACTION` 44건, `EARNINGS` 28건, `MACRO` 15건을 teacher gate로 추가 승격했다.
...
- raw 후보는 3,262개 종목까지 매칭되므로 다음 단계는 raw 후보를 종목별·라벨별로 검수해 supervised/gold 데이터로 승격하는 것이다.
- 후보 큐는 3,086개 종목을 포함하지만 약지도 기반 검수 대기 데이터이므로 gold label로 직접 사용하지 않는다.
