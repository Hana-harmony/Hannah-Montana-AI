...
- 설명 생성: 피어 선택은 hybrid ranker가 담당하고, headline/summary는 구조화된 peer fact를 근거로 생성하는 Structured RAG 설명기로 분리했다. 기본 serving은 deterministic grounded template이며, `HANNAH_GLOBAL_PEER_EXPLANATION_MODE=local_llm`과 OpenAI-compatible local endpoint가 켜진 경우에만 Qwen3 설명 LLM을 사용한다. LLM 출력은 display name, peer명, ticker, 섹터/산업/사업모델, 금지 투자문구 검증을 통과해야 하며 실패하면 template으로 fallback한다.
- Qwen3 설명 LLM: `data/training/global_peer_explanation_sft.jsonl` 3,967개 전종목 SFT 샘플로 `mlx-community/Qwen3-0.6B-4bit` LoRA를 학습했다. prompt version은 `global-peer-structured-rag-explainer-v3`, split은 train 3,571 / valid 198 / test 198이고, 500 iters, batch size 1, 8 layers, learning rate 1e-5 기준 final validation loss 0.014, test loss 0.000, test perplexity 1.000, peak memory 1.830GB를 기록했다.
- Codex 검수 보강: 회사명 표시용 `display_name`을 anchor에 추가하고, raw Qwen3가 display name을 누락하면 실제 API는 검수된 template으로 fallback한다. 알테오젠 raw LLM 직접 생성은 한국어명을 유지하는 한계가 있어 운영 노출은 guardrail 결과인 `"Alteogen Is The 'Halozyme Therapeutics' of South Korea — A Global Biotech Platform Leader"`를 사용한다.
- 산출물: `src/hannah_montana_ai/model_store/global_peer_ml.joblib`, `src/hannah_montana_ai/model_store/global_peer_qwen3_explainer_lora/adapters.safetensors`, `src/hannah_montana_ai/model_store/global_peer_qwen3_explainer_lora/adapter_config.json`, `reports/global-peer-training-report.json`, `reports/global-peer-ai-smoke-report.json`, `reports/global-peer-korea-industry-sync-report.json`, `reports/global-peer-fundamentals-sync-report.json`, `reports/global-peer-full-coverage-report.json`, `reports/global-peer-all-results.json`, `reports/global-peer-all-results.csv`, `reports/global-peer-explanation-llm-readiness.json`, `reports/global-peer-qwen3-explainer-training.json`, `docs/GLOBAL_PEER_ALL_RESULTS.md`, `data/reference/us_stock_universe.csv`, `data/reference/global_peer_fundamentals.csv`, `data/reference/korea_stock_industries.csv`
- 재학습: `uv run python scripts/sync_us_stock_universe.py`로 미국 universe를 갱신하고, `uv run python scripts/sync_global_peer_fundamentals.py`, `uv run python scripts/sync_korea_stock_industries.py`로 한국·미국 재무/규모/업종 dataset을 갱신한 뒤, `uv run python scripts/train_global_peer_model.py`, `uv run python scripts/build_global_peer_ai_smoke_report.py`, `uv run python scripts/build_global_peer_full_coverage_report.py`, `uv run python scripts/build_global_peer_all_results_report.py`, `uv run python scripts/build_global_peer_explanation_llm_dataset.py`, `uv run --extra llm-training python scripts/train_global_peer_qwen3_explainer.py`로 artifact와 report를 재생성한다.
