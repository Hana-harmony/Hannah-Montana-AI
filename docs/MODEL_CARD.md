...
- 설명 생성: 피어 선택은 hybrid ranker가 담당하고, headline/summary는 구조화된 peer fact를 근거로 생성하는 Structured RAG 설명기로 분리했다. 기본 serving은 deterministic grounded template이며, `HANNAH_GLOBAL_PEER_EXPLANATION_MODE=local_llm`과 OpenAI-compatible local endpoint가 켜진 경우에만 Qwen3 설명 LLM을 사용한다. LLM 출력은 종목명, peer명, ticker, 섹터/산업/사업모델, 금지 투자문구 검증을 통과해야 하며 실패하면 template으로 fallback한다.
- Qwen3 설명 LLM: `data/training/global_peer_explanation_sft.jsonl` 3,967개 전종목 SFT 샘플로 `mlx-community/Qwen3-0.6B-4bit` LoRA를 학습했다. split은 train 3,571 / valid 198 / test 198이고, 300 iters, batch size 1, 8 layers, learning rate 1e-5 기준 final validation loss 0.016, test loss 0.006, test perplexity 1.006, peak memory 1.728GB를 기록했다.
