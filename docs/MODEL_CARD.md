...
## 모델명
`financial-ml-tfidf-logreg-20260604163220`
...
- 이번 artifact 학습 샘플 수: 4,269
- supervised 학습 샘플 수: 3,609
- teacher-gated pseudo-label 학습 샘플 수: 660
...
- teacher confidence gate 통과 후 artifact 학습 승격 후보 수: 660건
- weak-label distillation 승격 수: 360건
- 종목 후보 큐 승격 수: 300건
- 종목 후보 큐 승격 종목 수: 300개
...
- 이벤트 태그 probability threshold는 기본 0.30으로 두고, 실제 뉴스 gold 기준으로 `CONTRACT` 0.26, `CORPORATE_ACTION` 0.18, `EARNINGS` 0.38, `MACRO` 0.34, `RISK` 0.52를 label별 calibration했다.
...
- 종목 후보 큐에서는 teacher gate와 종목별 quota를 통과한 `RISK` 150건, `CONTRACT` 150건만 이벤트 모델 학습에 추가했다.
- 종목 후보의 per-stock quota는 1건으로 제한해 300건이 300개 종목에 분산되도록 했다.
- 300종목 확장 후 threshold 미조정 모델은 실제 뉴스 gold event macro F1이 0.8888로 떨어져 폐기했고, label별 threshold 재보정 후 release gate를 통과했다.
...
- 현재 모델 버전: `financial-ml-tfidf-logreg-20260604163220`
- 전체 상태: `pass`
...
- 종목 후보 큐에서는 `RISK` 150건, `CONTRACT` 150건을 teacher gate로 추가 승격했다.
...
- 현재 artifact는 후보 큐 중 300개 종목의 300건만 teacher gate를 통과한 event-model-only pseudo-label로 제한 투입했다.
