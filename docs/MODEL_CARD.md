...
## 모델명
`financial-ml-tfidf-logreg-20260604224454`
...
- 이번 artifact 학습 샘플 수: 4,590
- supervised 학습 샘플 수: 3,609
- teacher-gated pseudo-label 학습 샘플 수: 981
- 실제 수집 raw 총량: 47,963건
- 실제 수집 원천: OpenDART 공시검색 25,966건
- 실제 수집 원천: Naver News Search 21,997건
- 합성 증강 샘플 수: 1,656건
...
- 실제 뉴스 gold 샘플 수: 80건
- 약지도 후보 수: 47,963건
- distillation 통과 후보 수: 4,971건
- teacher confidence gate 통과 후 artifact 학습 승격 후보 수: 981건
- weak-label distillation 승격 수: 360건
- 종목 후보 큐 승격 수: 621건
- 종목 후보 큐 승격 종목 수: 621개
...
- raw 후보에서 보수적 종목명·종목코드 매칭으로 확인한 종목 수: 2,929개
- 학습 승격 후보 큐 샘플 수: 9,888건
- 학습 승격 후보 큐 종목 수: 2,739개
...
- 종목 후보 큐에서는 teacher gate와 종목별 quota를 통과한 `RISK` 215건, `CONTRACT` 210건, `CAPITAL_ACTION` 120건, `CORPORATE_ACTION` 45건, `EARNINGS` 19건, `MACRO` 12건을 이벤트 모델 학습에 추가했다.
- 종목 후보의 per-stock quota는 1건으로 제한해 621건이 621개 종목에 분산되도록 했다.
- `stock-candidate-quota-experiment`에서 이전 500건/500종목, risk/contract 확장 764건/578종목, calibrated current release 621건/621종목 profile은 모두 release gate를 통과했다. current release는 실제 뉴스 gold gate를 통과하면서 후보 종목 수가 가장 많아 best profile로 선택했다.
...
- 47,963건 raw 후보 중 4,971건이 고신호 후보로 남았다.
- teacher confidence 또는 weak-label 합의 기준에서 3,558건이 탈락했다.
- weak-label distillation에서는 `RISK` 140건, `CONTRACT` 180건, `CORPORATE_ACTION` 40건을 student 이벤트 모델 학습에 승격했다.
- 종목 후보 큐에서는 `RISK` 215건, `CONTRACT` 210건, `CAPITAL_ACTION` 120건, `CORPORATE_ACTION` 45건, `EARNINGS` 19건, `MACRO` 12건을 teacher gate로 추가 승격했다.
...
- raw 후보는 2,929개 종목까지 매칭되므로 다음 단계는 raw 후보를 종목별·라벨별로 검수해 supervised/gold 데이터로 승격하는 것이다.
- 후보 큐는 2,739개 종목을 포함하지만 약지도 기반 검수 대기 데이터이므로 gold label로 직접 사용하지 않는다.
- coverage review plan은 2,000개 종목의 검수 과제를 만들지만 아직 검수자 승인이 없으므로 supervised coverage gate를 통과시키지 않는다.
- 현재 artifact는 후보 큐 중 621개 종목의 621건만 teacher gate를 통과한 event-model-only pseudo-label로 제한 투입했다.
