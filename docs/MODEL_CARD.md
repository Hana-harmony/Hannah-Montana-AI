...
## 모델명
`financial-ml-tfidf-logreg-20260604215055`
...
- 이번 artifact 학습 샘플 수: 4,549
- supervised 학습 샘플 수: 3,609
- teacher-gated pseudo-label 학습 샘플 수: 940
- 실제 수집 raw 총량: 44,997건
- 실제 수집 원천: OpenDART 공시검색 25,966건
- 실제 수집 원천: Naver News Search 19,031건
- 합성 증강 샘플 수: 1,656건
...
- 실제 뉴스 gold 샘플 수: 80건
- 약지도 후보 수: 44,997건
- distillation 통과 후보 수: 4,952건
- teacher confidence gate 통과 후 artifact 학습 승격 후보 수: 940건
- weak-label distillation 승격 수: 360건
- 종목 후보 큐 승격 수: 580건
- 종목 후보 큐 승격 종목 수: 580개
...
- raw 후보에서 보수적 종목명·종목코드 매칭으로 확인한 종목 수: 2,758개
- 학습 승격 후보 큐 샘플 수: 8,917건
- 학습 승격 후보 큐 종목 수: 2,567개
...
- 종목 후보 큐에서는 teacher gate와 종목별 quota를 통과한 `RISK` 181건, `CONTRACT` 208건, `CAPITAL_ACTION` 120건, `CORPORATE_ACTION` 43건, `EARNINGS` 16건, `MACRO` 12건을 이벤트 모델 학습에 추가했다.
- 종목 후보의 per-stock quota는 1건으로 제한해 580건이 580개 종목에 분산되도록 했다.
- `stock-candidate-quota-experiment`에서 이전 500건/500종목과 calibrated current release 580건/580종목 profile은 release gate를 통과했다. risk/contract 확장 716건/536종목 profile은 실제 뉴스 gold macro F1 0.8958로 gate를 통과하지 못해 현재 release에서 제외했다.
...
- 44,997건 raw 후보 중 4,952건이 고신호 후보로 남았다.
- teacher confidence 또는 weak-label 합의 기준에서 3,479건이 탈락했다.
- weak-label distillation에서는 `RISK` 140건, `CONTRACT` 180건, `CORPORATE_ACTION` 40건을 student 이벤트 모델 학습에 승격했다.
- 종목 후보 큐에서는 `RISK` 181건, `CONTRACT` 208건, `CAPITAL_ACTION` 120건, `CORPORATE_ACTION` 43건, `EARNINGS` 16건, `MACRO` 12건을 teacher gate로 추가 승격했다.
...
- raw 후보는 2,758개 종목까지 매칭되므로 다음 단계는 raw 후보를 종목별·라벨별로 검수해 supervised/gold 데이터로 승격하는 것이다.
- 후보 큐는 2,567개 종목을 포함하지만 약지도 기반 검수 대기 데이터이므로 gold label로 직접 사용하지 않는다.
- coverage review plan은 2,000개 종목의 검수 과제를 만들지만 아직 검수자 승인이 없으므로 supervised coverage gate를 통과시키지 않는다.
- 현재 artifact는 후보 큐 중 580개 종목의 580건만 teacher gate를 통과한 event-model-only pseudo-label로 제한 투입했다.
